...
install: check
	...
	test -f $(DESTDIR)$(SYSCONFDIR)/ryujin-iii-fand.conf || install -Dm644 ryujin-iii-fand.conf $(DESTDIR)$(SYSCONFDIR)/ryujin-iii-fand.conf
...
uninstall:
	...
	rm -f $(DESTDIR)$(SYSCONFDIR)/ryujin-iii-fand.conf
	udevadm control --reload-rules
	systemctl daemon-reload
enable:
